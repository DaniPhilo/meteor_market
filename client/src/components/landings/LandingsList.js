...
function LandingsList() {
  ...
  useEffect(() => {
    const init = async () => {
      const data = await fetchLandings(`https://pure-chamber-96021.herokuapp.com/api/astronomy/landings/${currentPage}?field=name&order=1`);
      if (data.response) {
  ...
  const handleSearchByName = async (event) => {
    ...
    setLoading(true);
    const data = await fetchLandings(`https://pure-chamber-96021.herokuapp.com/api/astronomy/landings/name/${event.target.name.value}`, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      credentials: 'include',
  ...
  const changeOrder = async (event) => {
    ...
    const data = await fetchLandings(`https://pure-chamber-96021.herokuapp.com/api/astronomy/landings/1?field=${event.target.id}&order=${order}`, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      credentials: 'include',
