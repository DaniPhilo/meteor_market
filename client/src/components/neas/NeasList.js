...
function NeasList() {
  ...
  useEffect(() => {
    const init = async () => {
      const data = await fetchNeas(`${process.env.REACT_APP_BACKEND}/api/astronomy/neas/${currentPage}?field=designation&order=1`);
      if (data.response) {
  ...
  const handleSearchByDesignation = async (event) => {
    ...
    setLoading(true);
    const data = await fetchNeas(`${process.env.REACT_APP_BACKEND}/api/astronomy/neas/designation/${event.target.designation.value}`);
    if (data.response) {
      setNumberOfDocs(data.count);
      setNeas(data.neas);
      setLoading(false);
  ...
  const changeOrder = async (event) => {
    ...
    const data = await fetchNeas(`${process.env.REACT_APP_BACKEND}/api/astronomy/neas/1?field=${event.target.id}&order=${order}`);
    if (data.response) {
      setNumberOfPages(Math.floor(data.count / 10));
      setNumberOfDocs(data.count);
      setNeas(data.neas);
